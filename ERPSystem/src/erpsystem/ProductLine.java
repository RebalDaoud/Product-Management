...
and if all of this did not occur then sleep(do not have any mission)
    ...
    public void run() {
        while (isRunning) {
            ...
            //If the line isn't working → just monitor
            if (lineState != LineState.ACTIVE) {
                sleepThread();
                continue;
            }

            //If there are no tasks → Monitor
            ...
            //If the task is canceled → remove it
            ...
            try {
                ...
                for (int i = 0; i < currentMission.getOrderedQuantity(); i++) {
                    ...
                    //If it is canceled during execution
                    if (currentMission.getState() == State.CANCELED) {
                        break;
                    }
                ...
                //Failed → Waiting without comment
                currentMission.setState(State.WAITING);
                lastErrorMessage = ex.getMessage();
                ErrorLogger.logError(ex);

                //We leave the task in the queue and go back to monitoring.
                sleepThread();
            }
...
//methods:
    ...
    //wait if low stock / It's for all the sleeps in the code
